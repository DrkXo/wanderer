...
WandererAudioHandler audioHandler<WandererAudioHandler>(AudioHandlerRef ref) =>
    throw UnimplementedError();

typedef CurrentMediaPosition = ({
  Duration position,
  Duration buffered,
  Duration? total
});

class WandererAudioHandler extends BaseAudioHandler {
  final _player = AudioPlayer();
  ConcatenatingAudioSource _playList = ConcatenatingAudioSource(
    children: [],
  );

  WandererAudioHandler() {
  ...
  Stream<CurrentMediaPosition> positionDataStream() =>
      Rx.combineLatest3<Duration, Duration, Duration?, CurrentMediaPosition>(
